...
class LLM:
    ...
    # Using the system prompt to attempt tokenization preservation
    def __call__(self, prompts: list[str], system_prompt: str = "Correct only the grammatical errors in the sentence below. Make the minimum number of changes necessary. IMPORTANT: Preserve the original word tokenization and spacing, especially around punctuation (e.g., ' ?', ' .', ' ,') and contractions (e.g., 'It \'s', 'do n\'t'). Output only the corrected sentence.", temperature: float = 0.1, max_tokens=150, **kwargs):
        results = []
...
# Updated regex to handle the table format: TP FP FN Prec Rec F0.5
# It captures the Prec, Rec, and F0.5 values from the line following the header.
score_pattern = re.compile(
    r"TP\s+FP\s+FN\s+Prec\s+Rec\s+F0\.5\n"  # Header line
    r"\d+\s+\d+\s+\d+\s+"                    # TP, FP, FN counts
    r"(\d+\.\d+)\s+"                         # Precision (Group 1)
    r"(\d+\.\d+)\s+"                         # Recall (Group 2)
    r"(\d+\.\d+)"                            # F0.5 (Group 3)
)
...
for llm in models_to_benchmark:
    ...
    try:
        ...
        # LLM output is used directly, relying on system prompt for tokenization
        corrected_sentences = llm(original_sentences)
        generation_time = time.time() - start_time
        print(f"Generation took: {generation_time:.2f} seconds")

        # --- Log Sample Outputs (Raw LLM Output) --- #
        if LOG_SAMPLE_OUTPUTS > 0:
            ...
            for i in range(min(LOG_SAMPLE_OUTPUTS, len(original_sentences))):
                ...
                print(f"Corr (Raw) [{i+1}]: {corrected_sentences[i]}") # Log raw output
                print("---")
            print("\n")
        ...
        # 3. Write CORRECTED sentences (RAW LLM Output) to a temporary file
        with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix=".txt", encoding='utf-8') as temp_cor_file:
            for sentence in corrected_sentences: # Use raw LLM output
                temp_cor_file.write(sentence + '\n')
            temp_cor_path = temp_cor_file.name

        # 4. Create hypothesis M2 file using errant_parallel (using subset orig and RAW cor)
